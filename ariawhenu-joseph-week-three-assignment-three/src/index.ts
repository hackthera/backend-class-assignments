import { students, type Student } from "./students.js";
 ...
 * 72. Get Report
 */
export function getReport(studentId: number): void {
  const student = findStudent(studentId);
  if (!student) throw new Error("Student not found");
  ...
  for (const [subject, grade] of Object.entries(student.grades)) {
    console.log(
      `${subject}: ${grade} (${getLetterGrade(grade)})`
    );
  }
 ...
 * 73. Class Ranking
 */
export function getClassRanking(): Student[] {
 ...
 */
export function classSummary(): void {
  ...
  subjects.forEach(subject => {
    ...
    const status =
      avg >= 75 ? "Doing well ✅" : "Needs improvement ⚠️";
